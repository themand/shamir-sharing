...
function showInfo() {
  console.log(`shamir-restore v${info.version}`);
  console.log('This tool restores a secret from shares using Shamir\'s Secret Sharing.');
}

function showUsage() {
  console.group('Usage:');
  console.log('shamir-restore [fileprefix] [-options]');
  console.groupEnd();
  console.log();
  ...
  console.log('   Writes restored secret to file. If no filename is specified, fileprefix is used');
  ...
  console.log('Execute: shamir-restore mysecret');
  process.exit(1);
}

function showError(message, code = 1) {
  console.log(`Error: ${message}`);
  console.log('For help: shamir-restore -help');
  process.exit(code);
}
...
if (params.filePrefix) {
  ...
  try {
    ...
    if (files.length === 1) {
      showError(`Only one share file ${files[0]} found. At least two shares are required for restore.`);
    }
